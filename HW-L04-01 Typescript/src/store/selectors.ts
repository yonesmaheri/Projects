...
import type { ExpenseState, TimeRange } from "./expenseSlice";

const selectExpense = (state: RootState): ExpenseState => state.expense;
...
const getTimeKey = (date: string, range: TimeRange): string => {
...
export const selectTimeSeries = createSelector(
  ...
  ({ transactions, timeRange }) => {
    const map: Record<string, { time: string; income: number; expense: number }> =
      {};
